...
bool KVCacheManager::allocate() {
    ...
    // === FIX #5: Zero-initialize KV cache for deterministic behavior ===
    // Without this, missing barriers cause attention to read garbage instead of
    // zero. Zero is a safe fallback that produces deterministic (if slightly
    // wrong) output rather than random garbage.
    for (uint32_t i = 0; i < nLayers; ++i) {
        auto& layer = layers[i];
        for (int kv = 0; kv < 2; ++kv) {
            VkDeviceMemory& mem = (kv == 0) ? layer.kMemory : layer.vMemory;
            void* mapped = nullptr;
            VkResult r = vkMapMemory(device, mem, 0, layerBytes, 0, &mapped);
            if (r == VK_SUCCESS && mapped) {
                std::memset(mapped, 0, layerBytes);
