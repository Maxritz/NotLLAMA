...
int main(int argc, char** argv) {
    ...
    if (argc < 3) {
        std::cout << "Usage: rdna4_llama <model.weights.json> <model.weights.bin> [prompt]\n";
        profiler.cleanup();
        ctx.cleanup();
        fclose(dbg);
        return 0;
    ...
    fprintf(stderr, "[kv_cache] context capped: %u -> %u (was %.1f GB for KV)\n",
            ...
            (double)model.contextLength * model.blockCount * model.headCountKv * headDim * 4 * 2 / (1024.0*1024*1024));
    fflush(stderr);
