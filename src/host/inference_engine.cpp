...
// Fused GEMM shaders skip the separate dequant pass — they read quantized weights directly.
// This eliminates the F32 staging buffer write/read, reducing bandwidth 3× for these formats.
static bool isFusedGemmFormat(QuantFormat fmt) {
    return fmt == QuantFormat::Q4_K || fmt == QuantFormat::Q6_K;
}

static const char* getGemmPipelineName(QuantFormat fmt) {
    switch (fmt) {
        case QuantFormat::Q4_K: return "gemm_q4k";
        case QuantFormat::Q6_K: return "gemm_q6k";
        default: return "gemm";
    }
}

static uint32_t getGemmWorkgroupCount(uint32_t n, QuantFormat fmt) {
    if (isFusedGemmFormat(fmt)) {
        return (n + 255) / 256;
    }
    return (n + 31) / 32;
}

// Returns the weight address for a GEMM dispatch.
// For fused formats (Q4_K/Q6_K): returns the quantized address directly — no dequant.
// For other formats: dispatches dequant and returns the F32 staging address.
static uint64_t prepareGemmWeight(Scheduler* sched, PipelineBuilder* pipes,
    uint64_t dequantBufAddr, size_t dequantBufSize,
    const ModelDesc& model, const std::string& name,
    const TensorDesc* tensor, QuantFormat fmt,
    size_t outOffset = 0) {
    if (!tensor) return 0;
    if (isFusedGemmFormat(fmt)) {
        // Fused GEMM reads quantized weights directly — return original address
        return tensor->gpuAddress;
    }
    if (fmt == QuantFormat::F16 || fmt == QuantFormat::F32) {
        return tensor->gpuAddress;
    }
    // Need dequant: dispatch to staging buffer
    return dequantWeightInBatch(sched, pipes, dequantBufAddr, dequantBufSize, model, name, outOffset);
}
...
uint32_t InferenceEngine::forwardPartial(uint32_t tokenId, uint32_t seqPos, uint32_t maxLayers) {
    ...
    size_t logitsSize = (size_t)std::max(model->vocabSize, model->feedForwardLength * 3u) * sizeof(float);
    ...
    for (uint32_t layer = 0; layer < layersToRun; ++layer) {
        ...
        // === Attention: Fused GEMM (skip dequant for Q4_K/Q6_K) ===
        // Norm weights are always F32 — dequant if needed.
        // Q, K, V, O use fused GEMM shaders when their format supports it.
        ...
        // Q projection — fused or regular
        const TensorDesc* qTensor = findTensor(*model, prefix + ".attn_q.weight");
        QuantFormat qFmt = qTensor ? qTensor->format : QuantFormat::F32;
        uint64_t qWAddr = prepareGemmWeight(scheduler, pipelines, dequantAddr, dequantCapacity, *model, prefix + ".attn_q.weight", qTensor, qFmt, 0);
        GemmPushConstants qPC = {normHidden, qWAddr, qRowAddr, 1, dim, dim, 1.0f, 0};
        scheduler->dispatchInBatch(pipelines->getPipeline(getGemmPipelineName(qFmt)), pipelines->getLayout("gemm"),
            &qPC, sizeof(qPC), getGemmWorkgroupCount(dim, qFmt), 1, 1);

        // K projection
        const TensorDesc* kTensor = findTensor(*model, prefix + ".attn_k.weight");
        QuantFormat kFmt = kTensor ? kTensor->format : QuantFormat::F32;
        uint64_t kWAddr = prepareGemmWeight(scheduler, pipelines, dequantAddr, dequantCapacity, *model, prefix + ".attn_k.weight", kTensor, kFmt, 0);
        GemmPushConstants kPC = {normHidden, kWAddr, kRowAddr, 1, kvDim, dim, 1.0f, 0};
        scheduler->dispatchInBatch(pipelines->getPipeline(getGemmPipelineName(kFmt)), pipelines->getLayout("gemm"),
            &kPC, sizeof(kPC), getGemmWorkgroupCount(kvDim, kFmt), 1, 1);

        // V projection
        const TensorDesc* vTensor = findTensor(*model, prefix + ".attn_v.weight");
        QuantFormat vFmt = vTensor ? vTensor->format : QuantFormat::F32;
        uint64_t vWAddr = prepareGemmWeight(scheduler, pipelines, dequantAddr, dequantCapacity, *model, prefix + ".attn_v.weight", vTensor, vFmt, 0);
        GemmPushConstants vPC = {normHidden, vWAddr, vRowAddr, 1, kvDim, dim, 1.0f, 0};
        scheduler->dispatchInBatch(pipelines->getPipeline(getGemmPipelineName(vFmt)), pipelines->getLayout("gemm"),
            &vPC, sizeof(vPC), getGemmWorkgroupCount(kvDim, vFmt), 1, 1);
        ...
        // FIX: pass BASE addresses (qAddr/kAddr), not row addresses. Shader computes its own offset.
        RopePushConstants ropePC = {qAddr, kAddr, seqLen, headDim, model->headCount, model->headCountKv, 10000.0f, 1.0f};
        ...
        // === FIX #1 (CRITICAL): Barrier ensures KV cache write is visible before attention reads ===
        // Without this barrier, the attention shader reads the KV cache before the write completes,
        // causing it to see stale/uninitialized data. This was the root cause of MaxAE 10-24.
        scheduler->barrierBetweenGroups(
            VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
            VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT);
        ...
        const TensorDesc* oTensor = findTensor(*model, prefix + ".attn_output.weight");
        QuantFormat oFmt = oTensor ? oTensor->format : QuantFormat::F32;
        uint64_t oWAddr = prepareGemmWeight(scheduler, pipelines, dequantAddr, dequantCapacity, *model, prefix + ".attn_output.weight", oTensor, oFmt, 0);
        GemmPushConstants outPC = {attnOutAddr, oWAddr, mlpOutAddr, 1, dim, dim, 1.0f, 0};
        scheduler->dispatchInBatch(pipelines->getPipeline(getGemmPipelineName(oFmt)), pipelines->getLayout("gemm"),
            &outPC, sizeof(outPC), getGemmWorkgroupCount(dim, oFmt), 1, 1);
        ...
        // === FFN: Fused GEMM (skip dequant for Q4_K/Q6_K) ===
        ...
        if (layer == 0) {
            fprintf(stderr, "[diag] L0 ffn norm dq: 0x%llx\n", (unsigned long long)addrFfnNorm_dq);
        }
        ...
        const TensorDesc* gateTensor = findTensor(*model, prefix + ".ffn_gate.weight");
        QuantFormat gateFmt = gateTensor ? gateTensor->format : QuantFormat::F32;
        uint64_t gateWAddr = prepareGemmWeight(scheduler, pipelines, dequantAddr, dequantCapacity, *model, prefix + ".ffn_gate.weight", gateTensor, gateFmt, 0);
        GemmPushConstants gatePC = {attnOutAddr, gateWAddr, gateScratchAddr, 1, hiddenDim, dim, 1.0f, 0};
        scheduler->dispatchInBatch(pipelines->getPipeline(getGemmPipelineName(gateFmt)), pipelines->getLayout("gemm"),
            &gatePC, sizeof(gatePC), getGemmWorkgroupCount(hiddenDim, gateFmt), 1, 1);

        const TensorDesc* upTensor = findTensor(*model, prefix + ".ffn_up.weight");
        QuantFormat upFmt = upTensor ? upTensor->format : QuantFormat::F32;
        uint64_t upWAddr = prepareGemmWeight(scheduler, pipelines, dequantAddr, dequantCapacity, *model, prefix + ".ffn_up.weight", upTensor, upFmt, 0);
        GemmPushConstants upPC = {attnOutAddr, upWAddr, upScratchAddr, 1, hiddenDim, dim, 1.0f, 0};
        ...
        const TensorDesc* downTensor = findTensor(*model, prefix + ".ffn_down.weight");
        QuantFormat downFmt = downTensor ? downTensor->format : QuantFormat::F32;
        uint64_t downWAddr = prepareGemmWeight(scheduler, pipelines, dequantAddr, dequantCapacity, *model, prefix + ".ffn_down.weight", downTensor, downFmt, 0);
        GemmPushConstants downPC = {interScratchAddr, downWAddr, mlpOutAddr, 1, dim, hiddenDim, 1.0f, 0};
        scheduler->dispatchInBatch(pipelines->getPipeline(getGemmPipelineName(downFmt)), pipelines->getLayout("gemm"),
            &downPC, sizeof(downPC), getGemmWorkgroupCount(dim, downFmt), 1, 1);
        ...
        // Diagnostic: read back dequant buffer after layer 0 completes (only norm weights, fused GEMM skips dequant)
        if (layer == 0 && dequantMemory && addrFfnNorm_dq && dequantAddr) {
            void* mapped = nullptr;
    ...
    uint32_t lmTransB = 0;
    QuantFormat lmFmt = QuantFormat::F32;
    if (addrLMHead) {
        const TensorDesc* lmTensor = findTensor(*model, "output.weight");
        lmFmt = lmTensor ? lmTensor->format : QuantFormat::F32;
        addrLMHead = prepareGemmWeight(scheduler, pipelines, dequantAddr, dequantCapacity, *model, "output.weight", lmTensor, lmFmt, 0);
    } else {
        addrLMHead = findTensorAddr(*model, "token_embd.weight");
        if (embedCacheReady) {
            addrLMHead = embedCacheAddr;
            lmFmt = QuantFormat::F32;
        } else {
            const TensorDesc* lmTensor = findTensor(*model, "token_embd.weight");
            lmFmt = lmTensor ? lmTensor->format : QuantFormat::F32;
            addrLMHead = prepareGemmWeight(scheduler, pipelines, dequantAddr, dequantCapacity, *model, "token_embd.weight", lmTensor, lmFmt, 0);
        }
        lmTransB = 1;
    }

    GemmPushConstants lmPC = {attnOutAddr, addrLMHead, logitsAddr, 1, model->vocabSize, dim, 1.0f, lmTransB};
    scheduler->dispatchInBatch(pipelines->getPipeline(getGemmPipelineName(lmFmt)), pipelines->getLayout("gemm"),
        &lmPC, sizeof(lmPC), getGemmWorkgroupCount(model->vocabSize, lmFmt), 1, 1);
